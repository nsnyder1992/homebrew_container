...
export function replace(document, file, options) {
  ...
  document = document.replaceAll(wikilinks, (node) => {
    if (node) {
      ...
      let linkpath = searchFile(dir, l);

      if (!linkpath)
        return `${start}[${text}](${l}${header ? `#${header}` : ""})`;
  ...
  document = document.replaceAll(mdImportlinks, (node) => {
    if (node) {
      let value = node;
      let arr = value.split("(");
  ...
  console.log(path.relative(path.join(process.cwd()), file?.history[0]));

  return document.replaceAll("@components", (node) => {
    if (node) {
      let relative = path.relative(path.join(process.cwd()), file?.history[0]);
      relative = relative.replaceAll("\\", "/");
      let dots = "";
      let i = relative.indexOf("/");
      while (i !== -1) {
        dots += "../";
        i = relative.indexOf("/", i + 1);
      }

      return dots + "components";
    }
